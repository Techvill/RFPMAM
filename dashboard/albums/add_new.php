...
<div class="container-fluid">
    <div class="col-md-12 text-center">
        ...
        <div class="col-md-6 col-md-offset-3" id="white-back">
            <br/>
            <br/>
            <p>You can create a singles album for any artist to be able to add new singles to the artist's
                singles collection. </p>
            <p class="text-danger text-justify">Please Note : Albums that will be created for an artist who is set on
                trending will automatically appear
                under trending albums
                , songs and trending singles!!.</p>
            ...
            if (isset($_POST['save'])) {
                ...
                $ST = "SELECT * FROM albums WHERE artist_id='$ArtistID' AND name_alb='$Name'";
                $RS = DBH::getInstance()->CRUD($ST);

                if ($RS->rowCount() > 0) {
                    {
                        $error[] = "The album you tried to add already exists for this artist in the database.";
                    }
                }
                if (empty($error)) {
                    ...
                    $move_file_pic = move_uploaded_file($pro_image['tmp_name'], $alb_covers . $profile_picture_url);
                    if ($move_file_pic) {

                        $INSERT = "INSERT INTO albums(name_alb,artist_id,cover) VALUES ('$Name','$ArtistID','$profile_picture_url')";
                        $RESULT = DBH::getInstance()->CRUD($INSERT);
                        if ($RESULT->rowCount() > 0) {
                            $info[] = "Album has been added successfully!!";
                        } else {
                            $error[] = "There was an error in adding a new album. Contact system admin.";
                        }
            ...
            <form id="login-form" autocomplete="on" enctype="multipart/form-data" role="form" method="POST" action=""
                ...
                <div class="form-group">
                    <label class="col-sm-4" for="inputEmail1">Album Name</label>
                    <div class="col-sm-8">
                ...
                <div class="form-group">
                    <label class="col-sm-4" for="inputEmail1">Album Art</label>
                    <div class="col-sm-8">
