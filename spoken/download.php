...
$ArtistID = '';
$SongArt = '';
$PST = "SELECT song_name,url,downloads,song_art,artist_name  FROM  spoken_word_artists,spoken_words 
WHERE spoken_word_artists.artist_id=spoken_words.artist_id  
                             AND spoken_words.id ='$id' ";
...
<head>
    ...
    echo "<link href='../IMedia/spoken/$SongArt' rel='icon' type='image/png'/>";

    ?>

    <meta property="og:site_name" content="Raised For a Purpose Music and Arts Ministry. "/>
    <meta property="og:title"
          content="<?php echo "Download live $songName by $Artist | RFP Music and Arts " ?>"/>
    <meta property="og:description"
          content="<?php echo "Stream and download music  by $Artist | RFP Music and Arts " ?>"/>
    <?php

    echo "<meta property='og:image' content='http://www.rfpmam.com/IMedia/spoken/$SongArt'/>";
    echo "<meta property='og:url' content='http://www.rfpmam.com/spoken/download.php?uid=$id'/>";
    ...
    <div class="container">
        <div class="col-md-6 col-md-offset-3" id="white-back">
            ...
            if (isset($_POST['download'])) {
                ...
                $UPDATE = "UPDATE spoken_words SET downloads ='$FinalDownloadCounter' WHERE id ='$id'";
                $PUPST = DBH::getInstance()->CRUD($UPDATE);

                if ($PUPST->rowCount() > 0) {
                    echo "<meta http-equiv='refresh' content='1;url=../DL/spoken/$URL'>";
                    echo "<p class='text-center'> Your download will start in a second. If not, use this link to download the
                                        <a download='true' href='../DL/spoken/$URL'>Download</a></p>";
                }

            }

            echo "<img src='../IMedia/spoken/$SongArt' id='art' class='img-responsive center-block'/><br/>";
